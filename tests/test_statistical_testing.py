...
def test_t_test(prepare_sample_data):
    ...
    assert (ttest_stats.loc[ttest_stats["variant"] == 1, "mean"] == ttest_stats.loc[ttest_stats["variant"] == 1, "mean_c"]).all()
...
def test_eval_ttest_significance(prepare_sample_data, p_threshold):
    ...
    assert (ttest_stats.loc[(ttest_stats["significance"] == "up") | (ttest_stats["significance"] == "down"), "p_value"] <= p_threshold).all()
